...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('cut.dwt', $cache_id))
    {
        ...
        $cut['share_url'] ='http://'.$_SERVER['HTTP_HOST']."/mobile/cut.php?act=view&id=".$id."&actuid=".$actuid."&u=".$_SESSION['user_id']  ."&scene_id=".$_SESSION['user_id'] ; //20160119 added by PRINCE 120029121
	    ...
	    if($cut['status_no'] == FINISHED || $cut['status_no'] == 0){
			...
			if($chk_join && $actuid==$_SESSION['user_id']){
				 ...
				 $right_action='我参加的';
				 $right_url="cut.php?act=view&id=$id&actuid=$u&u=$u&scene_id=$u";
			}else{
				 $right_action='参与活动';
...
elseif ($_REQUEST['act'] == 'join')
{
    ...
    /* 参加成功 跳转到活动详情页 */
    ecs_header("Location: cut.php?act=view&id=$id&actuid=$u&u=$u&scene_id=$u\n");
    exit;
}
