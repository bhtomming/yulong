...
elseif ($_REQUEST['act'] == 'share_list')
{
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
            $sql = "SELECT u.user_id, u.user_name, '$i' AS level, u.email, u.is_validated, u.user_money, u.frozen_money, u.rank_points, u.pay_points, u.reg_time,u.wxid,wu.nickname,wu.headimgurl ".
                    " FROM " . $GLOBALS['ecs']->table('users') ."  as u ".
                    " left join wxch_user as wu on wu.wxid=u.wxid ".
                    " WHERE u.user_id IN($up_uid)" .
                    " ORDER by level, u.user_id";
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['rank'])
        {
            ...
            {
                $ex_where .= " AND u.rank_points >= " . intval($row['min_points']) . " AND rank_points < " . intval($row['max_points']);
            }
        }
        ...
        $sql = "SELECT u.user_id, u.user_name, u.email, u.is_validated, u.user_money, u.frozen_money, u.rank_points, u.pay_points, u.reg_time,wu.nickname ".
                " FROM " . $GLOBALS['ecs']->table('users')
            ." as u LEFT JOIN wxch_user as wu on u.wxid = wu.wxid "
            . $ex_where .
