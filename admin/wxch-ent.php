...
if($_REQUEST['act'] == 'wxconfig') 
{
	...
	if(!empty($_POST['token']))
	{
		...
		if($ret)
		{
			sys_msg('尚网网络科技100提示您设置成功',0,$link);
		}
		else
		{
			sys_msg('尚网网络科技100提示您设置失败',0,$link);
		}
	}
	else
	{
	...
	if(!empty($_POST['hidden_type']))
	{
		...
		if(!empty($_FILES['bg_img']['tmp_name']))
		{
			if ((isset($_FILES['bg_img']['error']) && $_FILES['bg_img']['error'] == 0) || (!isset($_FILES['bg_img']['error']) && isset($_FILES['bg_img']['tmp_name']) && $_FILES['bg_img']['tmp_name'] != 'none'))
			{
				...
				//清空系统已生成的二维码和推广图片
				del_file(ROOT_PATH."images/qrcode/head/");
				del_file(ROOT_PATH."images/qrcode/");
				del_file(ROOT_PATH."qrcode/");
			}
			if (((isset($_FILES['bg_img']['error']) && $_FILES['bg_img']['error'] > 0) || (!isset($_FILES['bg_img']['error']) && isset($_FILES['bg_img']['tmp_name']) && $_FILES['bg_img']['tmp_name'] == 'none')) )
			{
...
elseif($_REQUEST['act'] == 'delcatch'){
	//清空系统已生成的二维码和推广图片
	del_file(ROOT_PATH."images/qrcode/head/");
	del_file(ROOT_PATH."images/qrcode/");
	del_file(ROOT_PATH."qrcode/");
	print_r("二维码缓存图片已清理完成");
	exit;
...
elseif($_REQUEST['act'] == 'look_erweima')
{
	...
	$bg_imgsrc = 'http://'.$this_url.'/data/qrcode/tianxin100.jpg';
	
	//$qr_imgsrc = 'http://'.$this_url.'/xiaojunectouch/qrcode/1450788973.jpg';
	$qr_imgsrc = 'http://'.$this_url.'/data/qrcode/demo.jpg';
	if(!getimagesize($qr_imgsrc))
	{
		print_r("请在项目文件夹下的data/qrcode下上传一张名为demo.jpg 的二维码图片，以便于预览");
		exit;
	}
	//$h_imgsrc='http://'.$this_url.'/xiaojunectouch/qrcode/1450788973_1.jpg';
	$h_imgsrc='http://'.$this_url.'/data/qrcode/headImg.jpg';
	...
	else
	{
		...
		$target = 'http://'.$this_url.'/data/qrcode/tianxin100.jpg';//背景图片
	}
	...
	$fontfile = '../data/qrcode/simsun.ttf';
