...
if ($_REQUEST['act'] == 'search_users')
{
    ...
    $sql = "SELECT user_id, user_name,nick_name FROM " . $ecs->table('users') .
            " WHERE user_name LIKE '%" . mysql_like_quote($keywords) . "%' OR nick_name LIKE '%" . mysql_like_quote($keywords) . "%' OR user_id LIKE '%" . mysql_like_quote($keywords) . "%'";
    $row = $db->getAll($sql);
    if($row){
        foreach($row as $k=>$v){
           $row[$k]['user_name'] = !empty($v['nick_name'])?$v['nick_name']:$v['user_name'];
        }
    }
...
function get_bonus_list()
{
    ...
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_sn, bt.type_name,u.nick_name ".
    ...
    foreach ($row AS $key => $val)
    {
        ...
        $row[$key]['user_name'] =!empty($val['nick_name'])?$val['nick_name']:$val['user_name'];
    }
