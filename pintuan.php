...
if ($_REQUEST['act'] == 'asynclist')
{
    ...
    if ($count > 0)
    {
        ...
        /* 计算总页数 */
        $max_page =  $page_count = ceil($count / $size);
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('pintuan_goods.dwt', $cache_id))
    {
        ...
        //头部广告banner
        $cat = $db->getRow("SELECT cat_name FROM " . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '".$goods['cat_id']."'");
        $smarty->assign('cat_ad',  current( get_wap_advlist('分类-'.$cat['cat_name'].'-top', 1)  ) );
//        die($cat['cat_name']);

        $pintuan['virtual_sold'] =$pintuan['virtual_sold']+$goods['sales_count'] ;
        $pintuan['share_url'] ="pintuan.php?act=view&act_id=".$pintuan_id."&u=".$userid ."&scene_id=".$userid ;
...
elseif ($_REQUEST['act'] == 'pt_view')
{
    ...
    if (!$smarty->is_cached('pintuan_view.dwt', $cache_id))
    {
        ...
        $pintuan['share_url'] ="pintuan.php?act=pt_view&pt_id=".$pintuan_id."&u=".$userid ."&scene_id=".$userid;
...
//    echo $sql ;
    while ($pintuan = $GLOBALS['db']->fetchRow($res)) {
        ...
        /* 处理链接 */
        $pintuan['url'] = 'pintuan.php?act=view&act_id=' . $pintuan['pintuan_id'] . '&u=' . $_SESSION['user_id'] ."&scene_id=".$_SESSION['user_id'];
        /* 加入数组 */
        $pt_list[] = $pintuan;
