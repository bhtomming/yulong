...
class Wechat{
	   ...
	   if(empty($_SESSION['user_id'])){
		   ...
		   //最后利用网页授权获得openid	   
			  ...
			  $code = $this->get_oauth2_code();
			   W_log( __LINE__ .'::: code : '.$code);
			  $wechatid = $this->get_oauth2_openid($code);
		   }
		   W_log( __LINE__ .'::: wechatid : '.$wechatid);
		   if(!empty($wechatid)){
			   
			   $userinfo = $GLOBALS['db']->getRow("SELECT * FROM ".$GLOBALS['ecs']->table('users')." WHERE `wxid` = '".$wechatid."'");
			   W_log( __LINE__ .'::: sql : '."SELECT * FROM ".$GLOBALS['ecs']->table('users')." WHERE `wxid` = '".$wechatid."'");
			    if(!$userinfo){
					W_log( __LINE__ .':::创建会员');
				   $u = trim($_REQUEST['u']);
  ...
  function get_oauth2_code()
  {
	  if(empty($_GET['code'])){
		...
		$redirect_uri = urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
		  W_log( __LINE__ .'::: redirect_uri:'.$redirect_uri);
		$para = array(
	         "appid"         => $row['appid'],
  ...
  function get_user_info($wechatid){
	 if(!empty($wechatid)) {
		 ...
		 W_log(__CLASS__.'|'.__METHOD__.'|'.__LINE__.print_r($w_user,true));
	   return $w_user;
	  }
  }
  ...
  function get_wechat_code(){
		if(empty($_GET['code'])){  
		   ...
		   $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$row['appid']."&redirect_uri=".$para['redirect_uri']."&response_type=code&scope=snsapi_base&state=123&connect_redirect=1#wechat_redirect";
		   url_redirect($url);
	   }
}
//取得openid
  ...
  function create_wechat_user($wechatid,$u){
	  /*
    ecs_users  与 wxch_user

     */
//	  $this->_addLog(__LINE__.' => '.__METHOD__);
	  if(empty($u)){
		  $u=0;
	  }
	  $time = time();
	  $user = $GLOBALS['db'] -> getRow("SELECT `wxid` FROM ".$GLOBALS['ecs']->table('users')." WHERE `wxid` = '$wechatid'");


	  if (empty($user)) {
		  if (!empty($wechatid)) {
//			  $this->_addLog(__LINE__.' =>  INSERT  ecs_users');

			  $q_name = $GLOBALS['db']->getOne("SELECT cfg_value FROM wxch_cfg WHERE cfg_name = 'q_name'");
			  $q_name .=  rand(10000,9999999);

			  $GLOBALS['db'] -> query("INSERT INTO ".$GLOBALS['ecs']->table('users')." (
					email,user_name,reg_time,last_login,parent_id,affiliate_id,wxid
					) VALUES ('','$q_name','$time','$time','$u','$u','$wechatid')");

			  //设置推荐人
			  if($u > 0){
				  $sales2info= $GLOBALS['db'] -> getRow("SELECT * FROM `ecs_users` WHERE `user_id` = '$u'");
				  $sales2_id=$sales2info['sales1_id'];
				  $sales3_id=$sales2info['sales2_id'];
				  $sales4_id=$sales2info['sales3_id'];
				  $sales5_id=$sales2info['sales4_id'];

				  $GLOBALS['db'] -> query( 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $u . ",sales1_id = '$u',sales2_id = '$sales2_id',sales3_id='$sales3_id',sales4_id='$sales4_id',sales5_id='$sales5_id'  WHERE wxid =  '$wechatid'") ;

			  }

		  }
	  } else {
//		  $this->_addLog(__LINE__.' =>  UPDATE  ecs_users');
		  $GLOBALS['db'] -> query("UPDATE  ".$GLOBALS['ecs']->table('users')." SET  `last_login` = '$time', WHERE  `wxch_user`.`wxid` = '$wechatid';");
	  }

		$ret = $GLOBALS['db'] -> getRow("SELECT `wxid` FROM `wxch_user` WHERE `wxid` = '$wechatid'");
		if (empty($ret)) {
			if (!empty($wechatid)) {
			return	$GLOBALS['db'] -> query("INSERT INTO `wxch_user` (`subscribe`, `wxid` , `dateline`,`affiliate`) VALUES ('0','$wechatid','$time','$u')");
			} 
		} else {
  ...
  function update_userinfo($wechatid){
      ...
      if($user && (empty($user['nick_name']))){	
	     $wuser_info = $this->get_user_info($wechatid);
		    ...
		    $sql = "UPDATE wxch_user SET  subscribe='".$wuser_info[subscribe]."',nickname='".$wuser_info[nickname]."',headimgurl='".$wuser_info[headimgurl]."',subscribe_time='".$wuser_info[subscribe_time]."',sex='".$wuser_info[sex]."' where wxid='".$wechatid."'";
		    $GLOBALS['db']->query($sql);
		  $sql = "UPDATE ecs_users SET  is_subscribe='".$wuser_info[subscribe]."',nicheng='".$wuser_info[nickname]."',nick_name='".$wuser_info[nickname]."',headpic_thumb='".$wuser_info[headimgurl]."',subscribe_time='".$wuser_info[subscribe_time]."'  where wxid='".$wechatid."'";
		  $GLOBALS['db']->query($sql);
	  }
  }
