...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
		...
		if(!empty($val['fencheng'])){
          ...
          // LHF 2018-11-15  添加修改 订单拿出一定比例出来分成
           $affiliate['config']['rebate_ratio'] = isset($affiliate['config']['rebate_ratio'])?$affiliate['config']['rebate_ratio']:0;
           $total['fencheng'] = $affiliate['config']['rebate_ratio'] * $total['fencheng'] / 100 ;
