...
function get_regions_json($parent = 0)
{
    $areas = null ;
    $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['ecs']->table('region') .
    ...
    if($list){
        foreach ($list as $k=>$v){
            $areas[$k]['value'] = $v['region_id'] ;
            $areas[$k]['text'] = $v['region_name'] ;
            $children = get_regions_json($v['region_id']);
            if($children){
                $areas[$k]['children'] =$children ;
            }
        }
    }
    return $areas ;
    ...
    switch ($app)
    {
        ...
        case 'goods':
            ...
            else
            {
				...
				if(!empty($userid)){
					$uri = $rewrite ? 'goods-' . $gid : 'goods.php?id=' . $gid."&u=".$userid."&scene_id=".$userid;
				}else{
...
/*
 * 生成二维码  如果存在 则返回false ，不存在 则生成 并返回路径
 * */
function createQR($url='',$fname = ''){
    ...
    if(!file_exists(ROOT_PATH.$jpg_ewmPath)){
        $ewmPath = str_replace('.jpg','.png',$ewmPath);
        QRcode::png($value,ROOT_PATH.$ewmPath , $errorCorrectionLevel, $matrixPointSize, 2);
        png2jpg(ROOT_PATH. $ewmPath,true);
        str_replace('.png','.jpg',$ewmPath);
        return  ROOT_PATH.$ewmPath;
    }
    return false;
...
/*
 * 获取头像  如果存在 则返回false ，不存在 则生成 并返回路径
 * */
function createFaceImg ($fname,$user_id){
    $time = substr($fname,0,-4);
    $userInfo = $GLOBALS['db']  -> getRow("SELECT * FROM  ".$GLOBALS['ecs']->table( "users")." as u
        left join wxch_user as wu on wu.wxid = u.wxid WHERE  u.user_id = '".$user_id."'");
    //头像获取
    $h_imgsrc= "images/qrcode/head/".$time.".jpg";
    if( !file_exists(ROOT_PATH . $h_imgsrc) )
    {
...
function scerweima1($fname,$user_id)
{
    ...
    if(!file_exists( ROOT_PATH.'qrcode/'.$fname) ){
        ...
        $h_imgs = $h_name;
        //背景图片
        if( file_exists( ROOT_PATH . $bg_img) )
        {
            $target =ROOT_PATH  . $bg_img ;//背景图片 mobile下的qrcode 文件下
        }
        else
        {
            $target =ROOT_PATH. 'data/qrcode/tianxin100.jpg';//背景图片
        }
        ...
        $fontfile =ROOT_PATH. "data/qrcode/simsun.ttf";
...
function png2jpg($srcPathName, $delOri=true,$pw=false,$ph=false)
{
    ...
    if ($srcFileExt == 'png') {
        ...
        $pw = $pw?$pw: $photoSize[0];
        $ph =$ph?$ph: $photoSize[1];
