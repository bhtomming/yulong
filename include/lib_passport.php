    ...
    else
    {
        ...
        if (isset($affiliate['on']) && $affiliate['on'] == 1)
        {
            ...
            if ($up_uid)
            {
                ...
                //设置推荐人
                $sales2_sql="SELECT * FROM `ecs_users` WHERE `user_id` = '$up_uid'";
                $sales2info= $GLOBALS['db'] -> getRow($sales2_sql);
                $sales2_id=$sales2info['sales1_id'];
                $sales3_id=$sales2info['sales2_id'];
                $sales4_id=$sales2info['sales3_id'];
                $sales5_id=$sales2info['sales4_id'];

                $sql = 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ",sales1_id = '$up_uid',sales2_id = '$sales2_id',sales3_id='$sales3_id',sales4_id='$sales4_id',sales5_id='$sales5_id'  WHERE user_id =  " . $_SESSION['user_id'];
