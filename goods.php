...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $goods['comment_count'] = get_comment_count($goods_id);
        ...
        if(strstr($goods['goods_weight'],0,1) == 0) unset($goods['goods_weight']);
//        echo '<pre>';
//        print_r($_SESSION['user_id']);
//print_r($goods);die;
        $goods['url'] = 'http://'.   $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] .'&u='.$_SESSION['user_id'].'&scene_id='.$_SESSION['user_id'];
        $goods['img'] = 'http://'.   $_SERVER['SERVER_NAME'] . $goods['goods_thumb'] ;
       $goods['goods_desc'] = str_replace('src="/images/', 'src="'.$config['site_url'].'images/', $goods['goods_desc']); //修复产品详情的图片 by wang
        $smarty->assign('goods',              $goods);
        $smarty->assign('goods_id',           $goods['goods_id']);
        $smarty->assign('promote_end_time',   $goods['gmt_end_time']);
        $smarty->assign('sales_count',        get_goods_sales_count($goods_id)); // by wang
        ...
        assign_template('c', $catlist);
         ...
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id FROM " .$ecs->table('goods'). " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods_id . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
        ...
        $next_gid = $db->getOne("SELECT max(goods_id) FROM " . $ecs->table('goods') . " WHERE cat_id=".$goods['cat_id']." AND goods_id < ".$goods_id . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
        if (!empty($next_gid))
        {
...
$smarty->assign('goods_n',$goods_n);
		...
		if(!empty($userid)){
			...
			$url=$config['site_url']."goods.php?id=".$goods_id."&u=".$userid."&scene_id=".$userid;
			$url_home=$config['site_url']."mobile/";
			//20141204新增分享返积分
			$dourl=$config['site_url']."re_url.php?user_id=".$userid;
			}else{
					$url="";
					//20141204新增分享返积分
					$dourl="";
