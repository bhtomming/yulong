...
elseif ($_REQUEST['step'] == 'consignee')
{
 ...
 //   die($_SESSION['user_id']) ;
    if( $_GET['default'] && $_SESSION['user_id'] > 0 ){
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET address_id = '$_GET[address_id]' WHERE user_id = '$_SESSION[user_id]'";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $smarty->assign('consignee_list_json', json_encode($consignee_list, JSON_UNESCAPED_UNICODE));
...
elseif($_REQUEST['step'] == 'ok')
{
...

    {
        ...
        file_put_contents(ROOT_PATH . DATA_DIR . '/pay_online.txt', 'order_amount:'.$order['order_amount']."\n".print_r($pay_online,true) ."\n\n");
