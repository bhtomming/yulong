...
elseif ($action == 'order_detail')
{
   ...
   // $order['pay_online'];
    file_put_contents(ROOT_PATH . DATA_DIR . '/pay_online.txt', print_r($order['pay_online'],true));
...
elseif ($action == 'address_list')
{
    ...
    foreach ($consignee_list AS $region_id => $consignee)
    {
        ...
        $consignee_list[$region_id]['area'] =  get_region_name($consignee['province']) . '  '. get_region_name($consignee['city']) .'  '.  get_region_name($consignee['district']) ;
...
elseif ($action == 'act_edit_address')
{
    ...
    if($_GET['flag'] == 'display'){
        ...
        foreach ($consignee_list AS $region_id => $vo)
        {
            if($vo['address_id'] == $id){
                $consignee = $vo;
                $vo['area'] = get_region_name($vo['province']) . '  '. get_region_name($vo['city']) .'  '.  get_region_name($vo['district']) ;
                $smarty->assign('consignee', $vo);                
            }
        }

//        $area = get_regions_json(1);
//
//echo (json_encode($area,JSON_UNESCAPED_UNICODE));die;
...
elseif ($action == 'async_message_list'){
    ...
    if(is_array($message_list)){
        foreach($message_list as $key=>$vo){
            $re_message = $vo['re_msg_content'] ? '<tr><td style="background: #fafafa;">'.$_LANG['shopman_reply'].' ('.$vo['re_msg_time'].')<br>'.$vo['re_msg_content'].'</td></tr>':'';
            $asyList[] = array(
                'message' => '<table width="100%" style="border: 1px solid #dfdfdf;" border="0" cellpadding="5" cellspacing="0" class="ectouch_table_no_border">
            <tr>
                <td style="background: #efefef;"><span style="float:right"><a href="user.php?act=del_msg&id='.$key.'&order_id='.$vo['order_id'].'" onclick="if (!confirm(\''.$_LANG['confirm_remove_msg'].'\')) return false;" style="color:#1CA2E1">删除</a></span>'.$vo['msg_type'].'：'.$vo['msg_title'].' - '.$vo['msg_time'].' </td>
            </tr>
            <tr>
...
elseif ($action == 'point_order')
{
    ...
    foreach ($list as $k => $v ){
        $list[$k]['number'] =  $k+1;
        $v['name'] = !empty($v['nicheng'])? $v['nicheng']:$v['nick_name'];
        $list[$k]['name'] = !empty($v['name'])?$v['name']:$v['user_name'];
    }
//    echo '<pre>';
//    var_dump($list);die;
    $user_id=$_SESSION['user_id'];
    ...
    if($paiming <= 1000){
        $pm = "第 $paiming 名";
    }elseif($paiming > 1000 && $paiming <5000){
    $pm = "1千+";
    }elseif($paiming > 5000 && $paiming <10000){
        $pm = "5千+";
    }elseif($paiming >= 10000 && $paiming <50000){
        $pm = "1万+";
    }elseif($paiming >= 50000 && $paiming <100000){
        $pm = "5万+";
    }else{
        $pm = "10万+";
    }
    $smarty->assign('row', $row );
    $smarty->assign('paiming', $pm );
