...
<?php echo $this->smarty_insert_scripts(array('files'=>'common.js,jquery.json.js,shopping_flow.js')); ?>
...
<script type="text/javascript" src="<?php echo $this->_var['ectouch_themes']; ?>/js/ectouch.js?v=1.00"></script>
</head>
...
?>
  <section class="order_box padd1 radius10" style="padding-top:0;padding-bottom:0;">
    <div class="table_box2 table_box">
      <form style="padding:8px;" action="flow.php?step=consignee&default=1&address_id=<?php echo $this->_var['consignee']['address_id']; ?>" method="post" name="theForm" id="theForm" onSubmit="return checkConsignee(this)">
  ...
  <section id="editForm" class="wrap">
      ...
      <form action="flow.php" method="post" name="theForm" onsubmit="return checkConsignee(this)">
        <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
          <tr>
            <td width="70">收货人</td>
            <td align="left" ><input name="consignee" placeholder="<?php echo $this->_var['lang']['consignee_name']; ?><?php echo $this->_var['lang']['require_field']; ?>" type="text" class="inputBg_touch" value="" /> </td>
          </tr>
          <tr>
            <td>联系电话</td>
            <td align="left" ><input placeholder="<?php echo $this->_var['lang']['phone']; ?><?php echo $this->_var['lang']['require_field']; ?>" name="tel" type="text" class="inputBg_touch" value="" /></td>
          </tr>
          <tr>
            <td>电子邮箱</td>
            <td align="left" ><input name="email" placeholder="<?php echo $this->_var['lang']['email_address']; ?>" type="text" class="inputBg_touch" value="" /></td>
          </tr>
          <tr>
            <td><?php echo $this->_var['lang']['country_province']; ?></td>
            <td align="left" >
               <input name="area" placeholder="请选择地区" id="area" type="text" class="inputBg_touch" value="<?php echo $this->_var['consignee']['area']; ?>" />
          ...
          <tr style="display: none;">
            <td>邮政编码</td>
            <td align="left" ><input placeholder="<?php echo $this->_var['lang']['postalcode']; ?>" name="zipcode" type="text" class="inputBg_touch" value="" /></td>
          </tr>
          <tr>
 ...
 <link rel="stylesheet" href="\themes\tianxin100/css/mui.min.css?v=1.1.1">
    
    <link rel="stylesheet" type="text/css" href="css/app.css?v=1.1.1" />
    <link href="\themes\tianxin100/css/mui.picker.css?v=1.1.1" rel="stylesheet" />
    <link href="\themes\tianxin100/css/mui.poppicker.css?v=1.1.1" rel="stylesheet" />
<style>
.table_box dd.cener {text-align: center;}
.table_box dd.cener  button{display: inline-block; margin: 0 auto;}
#editForm {position:fixed; top: 46px;left: 0px; width: 100%; height: 100%; display: none;     background-color: #efeff4;}
</style>
<script src="\themes\tianxin100\js/mui.min.js"></script>
    <script src="\themes\tianxin100\js/mui.picker.js"></script>
    <script src="\themes\tianxin100\js/mui.poppicker.js"></script>
    <script src="\themes\tianxin100\js/city.data.js" type="text/javascript" charset="utf-8"></script>
    <script src="\themes\tianxin100\js/city.data-3.js" type="text/javascript" charset="utf-8"></script>
    <script>
      var consigneeList = <?php echo $this->_var['consignee_list_json']; ?>;
      $('#addNew').click(function(){
       ...
       $('.editNew').click(function(){
           ...
           var num =  $(this).attr('data-num');
            $('#editForm').find('input[name="address_id"]').val( consigneeList[num].address_id);
           $('#editForm').find('input[name="consignee"]').val( consigneeList[num].consignee);
           $('#editForm').find('input[name="tel"]').val( consigneeList[num].tel);
           $('#editForm').find('input[name="email"]').val( consigneeList[num].email);
           $('#editForm').find('input[name="area"]').val( consigneeList[num].area);
           $('#editForm').find('input[name="address"]').val( consigneeList[num].address);
           $('#editForm').find('input[name="province"]').val( consigneeList[num].province);
           $('#editForm').find('input[name="city"]').val( consigneeList[num].city);
           $('#editForm').find('input[name="district"]').val( consigneeList[num].district);
           console.log(consigneeList[num].consignee);
        });
        ...
        $.ready(function() {
          ...
          showCityPickerButton.addEventListener('tap', function(event) {
            cityPicker3.show(function(items) {
              doc.getElementById('province').value = _getParam(items[0], 'value');
              doc.getElementById('city').value = _getParam(items[1], 'value');
              doc.getElementById('district').value = _getParam(items[2], 'value');
              console.info(_getParam(items[0], 'value'));
...
?>
		  <p class ="<?php if ($this->_var['order']['shipping_id'] == $this->_var['shipping']['shipping_id']): ?>check<?php endif; ?> ">

            <input name="shipping" type="radio" class="radio" id="shipping_<?php echo $this->_var['shipping']['shipping_id']; ?>" style="display: none;" value="<?php echo $this->_var['shipping']['shipping_id']; ?>" <?php if ($this->_var['order']['shipping_id'] == $this->_var['shipping']['shipping_id']): ?>checked="true"<?php endif; ?> supportCod="<?php echo $this->_var['shipping']['support_cod']; ?>" insure="<?php echo $this->_var['shipping']['insure']; ?>" style="vertical-align:middle; " /><label for="shipping_<?php echo $this->_var['shipping']['shipping_id']; ?>"> <?php echo $this->_var['shipping']['shipping_name']; ?> </label> 
           </p>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
?>
         ...
         <p class ="<?php if ($this->_var['order']['pay_id'] == $this->_var['payment']['pay_id']): ?>check<?php endif; ?> "  >
           <input type="radio" style="display: none;" class="radio" name="payment" id="payment_<?php echo $this->_var['payment']['pay_id']; ?>" value="<?php echo $this->_var['payment']['pay_id']; ?>" <?php if ($this->_var['order']['pay_id'] == $this->_var['payment']['pay_id']): ?>checked<?php endif; ?> isCod="<?php echo $this->_var['payment']['is_cod']; ?>"  <?php if ($this->_var['cod_disabled'] && $this->_var['payment']['is_cod'] == "1"): ?>disabled="true"<?php endif; ?> style="vertical-align:middle" /><label for="payment_<?php echo $this->_var['payment']['pay_id']; ?>"><?php echo $this->_var['payment']['pay_name']; ?></label>   
           </p>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
		</div>
        <?php else: ?>
...
?>
		...
		<div class="dl_box" id="package_box" style="display:none;">
         <p <?php if ($this->_var['order']['pack_id'] == 0): ?>class="check"<?php endif; ?>> 
          <input type="radio" class="radio"  name="pack" value="0" id="pack_0" <?php if ($this->_var['order']['pack_id'] == 0): ?>checked="true"<?php endif; ?> onclick="selectPack(this)" /><label for="pack_0"><?php echo $this->_var['lang']['no_pack']; ?></label></p>
...
?>
		  <p  <?php if ($this->_var['order']['pack_id'] == $this->_var['pack']['pack_id']): ?>class="check"<?php endif; ?> >
        <input type="radio" class="radio" name="pack" id="pack_<?php echo $this->_var['pack']['pack_id']; ?>" value="<?php echo $this->_var['pack']['pack_id']; ?>" <?php if ($this->_var['order']['pack_id'] == $this->_var['pack']['pack_id']): ?>checked="true"<?php endif; ?> onclick="selectPack(this)" /><label for="pack_<?php echo $this->_var['pack']['pack_id']; ?>"><?php echo $this->_var['pack']['pack_name']; ?>[<?php echo $this->_var['pack']['format_pack_fee']; ?>]</label></p>
         <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?> 	
		</div>
        <?php endif; ?> 
		...
		<div class="dl_box" id="card_box" style="display:none;">
           <p <?php if ($this->_var['order']['card_id'] == 0): ?>class="check"<?php endif; ?>> 
            <input type="radio" class="radio"  name="card" value="0"  id="card_0" <?php if ($this->_var['order']['card_id'] == 0): ?>checked="true"<?php endif; ?> onclick="selectCard(this)" /><label for="card_0"><?php echo $this->_var['lang']['no_card']; ?></label></p>
...
?>
           <p  <?php if ($this->_var['order']['card_id'] == $this->_var['card']['card_id']): ?>class="check"<?php endif; ?>>
            <input type="radio" class="radio"  name="card"  id="card_<?php echo $this->_var['card']['card_id']; ?>" value="<?php echo $this->_var['card']['card_id']; ?>" <?php if ($this->_var['order']['card_id'] == $this->_var['card']['card_id']): ?>checked="true"<?php endif; ?> onclick="selectCard(this)"  /><label for="card_<?php echo $this->_var['card']['card_id']; ?>"><?php echo $this->_var['card']['card_name']; ?>[<?php echo $this->_var['card']['format_card_fee']; ?>]</label>
           </p>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
		</div>
        <?php endif; ?>         
    ...
    <div class="b_no">
            <dl>
              <dd><?php echo $this->_var['lang']['your_surplus']; ?></dd>
              <dd class="weiruan " bgcolor="#ffffff"><input name="surplus" type="text" class="inputBg" id="ECS_SURPLUS" size="10" value="<?php echo empty($this->_var['your_surplus']) ? '0' : $this->_var['your_surplus']; ?>"disabled="disabled" />
              </dd>
            </dl>
    </div>  
...
.content a {display: block; height: 1rem;margin: 10px auto; background:#99cc33 ; color:#FFF;}
</style>
<header id="header">
