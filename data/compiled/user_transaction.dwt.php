...
?>
    ...
    <section class="wrap">
      <form name="formEdit" style="display: none;" action="user.php" method="post" onSubmit="return userEdit()">
...
?>
    ...
    <?php endif; ?> 
     ...
     <?php if ($this->_var['action'] == 'tuiguang'): ?>
        ...
        <section class="wrap">
              <div class="table_box table_box2" >               
               <img src="" id="qrImg" style="width:100%;height:auto;display:none" >              
              </div>
        </section>
      <script>
      var qrImg = '<?php echo $this->_var['tuiguang']['qr_path']; ?>';
      getQRshareImg({qrImg:qrImg});
      function getQRshareImg(param)
      {
          $.post('/user.php?act=tuiguang&qr=1',param,function(data){
             
              data = JSON.parse(data) ;
              if(parseInt(data.code) > 0)
              {
                  getQRshareImg({qrImg:data.msg});
                  return false;
              }
              $('#qrImg').attr('src',data.img).show();
              //alert(data.http);
          })
     }
      </script>
...
?>
    ...
    <header id="header">
     ...
     <style>
      .orderBtn{width: 100%;display: -webkit-box;}
      .orderBtn li {-webkit-box-flex: 1;text-align: center;padding: 3px;box-sizing: border-box;}
      .orderBtn li span,  .orderBtn li a {border-radius:5px;}
     </style>
    <section class="wrap order_list">
      ...
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
        ...
        <tr>
          ...
          <ul class="orderBtn">
            <li class="order_handler"></li>
            <li class="order_tracking"></li>
            <li class="order_back"></li>
          </ul>
          </td>
        </tr>
...
?>
    ...
    jQuery(function($){
		...
		$(window).scroll(function () {
		    $('.order_handler').each(function(k,v){if($(this).html().length <5) $(this).hide();})
		    $('.order_tracking').each(function(k,v){if($(this).html().length <5) $(this).hide();})
		    $('.order_back').each(function(k,v){if($(this).html().length <5) $(this).hide();})
...
?>
    ...
    <style>
.ectouch_table {background: #f3f3f3; padding: 0 5px; border-radius: 0.5rem;}
.ectouch_table .btn {text-align: center;}
.ectouch_table .btn a {background: #ec5151; padding: 0.1rem 0.5rem; margin: 0.1rem 0.5rem 0.3rem; display: inline-block; color: #fff;}
.ectouch_table .btn a.green {background: #99cc33;}
.ectouch_table .btn a.red {background: #ec5151;}
  </style>
    <header id="header">
      <div class="header_l header_return"> <a class="ico_10" href="user.php"> 返回 </a> </div>
...
?>
      ...
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
        ...
        <tr>
          <td align="right"><?php echo $this->_var['lang']['detailed_address']; ?>：</td>
          <td align="left"> <?php echo $this->_var['consignee']['area']; ?> <?php echo htmlspecialchars($this->_var['consignee']['address']); ?> 
        ...
        <?php if ($this->_var['consignee']['email'] != ""): ?>
        <tr>
          <td align="right"><?php echo $this->_var['lang']['email_address']; ?>：</td>
        ...
        <tr>
          <td colspan="2" class="btn">
            <a class="green" href="user.php?act=act_edit_address&id=<?php echo $this->_var['consignee']['address_id']; ?>&flag=display">编辑</a>  
            <a class="red" href="user.php?act=drop_consignee&id=<?php echo $this->_var['consignee']['address_id']; ?>" onClick="return confirm('您真的要删除该地址吗？');">删除</a></td>
        </tr>
      </table>
      </section>
      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      ...
      <form action="user.php" method="post" name="theForm" onsubmit="return checkConsignee(this)">
        <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
          ...
          <tr>
            ...
            <td align="left" >
               <input name="area" placeholder="请选择地区" id="area" type="text" class="inputBg_touch" value="<?php echo htmlspecialchars($this->_var['consignee']['area']); ?>" />
               <input type="hidden" name="country" value="1" />
                <input type="hidden" id="province" name="province" value="<?php echo $this->_var['consignee']['province']; ?>" />
                <input type="hidden" id="city" name="city" value="<?php echo $this->_var['consignee']['city']; ?>" />
                <input type="hidden" id="district" name="district" value="<?php echo $this->_var['consignee']['district']; ?>" />
