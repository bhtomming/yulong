...
<table width="100%" cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td><?php echo empty($this->_var['order']['user_name']) ? $this->_var['lang']['anonymous'] : $this->_var['order']['user_name']; ?> <?php if ($this->_var['order']['user_id'] > 0): ?>[ <a href="users.php?act=edit&id=<?php echo $this->_var['order']['user_id']; ?>" onclick="staticbar();return false;"><?php echo $this->_var['lang']['display_buyer']; ?></a> ] [ <a href="user_msg.php?act=add&order_id=<?php echo $this->_var['order']['order_id']; ?>&user_id=<?php echo $this->_var['order']['user_id']; ?>"><?php echo $this->_var['lang']['send_message']; ?></a> ]<?php endif; ?></td>
...
<!--申请退货理由 str-->
<?php if ($this->_var['order']['order_status'] == 7 || $this->_var['order']['order_status'] == 4): ?>
<div class="list-div" style="margin-bottom: 5px">
<table width="100%" cellpadding="3" cellspacing="1">
  <tr>
    <th>退货理由</th>
  ...
  <tr>
    <td><div align="center"><strong><?php echo $this->_var['order']['back_reason']; ?></strong></div></td>
  </tr>
</table>
</div>
<?php endif; ?>
...
<!-- 一键发货end-->  
    ...
    <?php if ($this->_var['operable_list']['confirm']): ?>
      <input name="confirm" type="submit" value="<?php echo $this->_var['lang']['op_confirm']; ?>" class="button" />
        ...
        <?php if ($this->_var['order']['extension_code'] == "pintuan"): ?>备注：拼团未成功前，不能发货。(<a href="pintuan.php?act=detail_view&pt_id=<?php echo $this->_var['order']['pt_id']; ?>" target="_blank">当前状态：<?php echo $this->_var['order']['pintuan_status']; ?></a>)<?php endif; ?>
...
<!-- add 物流快递跟踪查询插件 免申请接口 begin -->
<div class="blank"></div>
<style>
.result-box{background-color:#FFFFFF;overflow:hidden;font-family:Tahoma,Arial,sans-serif;font-size:12px;border:1px solid #B9D0F0;line-height:24px;}
.result-box table{width:100%;border-collapse:collapse;border-spacing:0;}
.result-box table td{height:30px;color:#333333;border-bottom:1px solid #E3E3E3;}
...
.result-box table .td2 strong{margin-left:5px;padding:0 1px;font-size:10px;color:#FFFFFF;background-color:#FF0000;}
</style>
